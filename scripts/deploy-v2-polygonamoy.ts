...
async function main() {
    try {
        ...
        const tokenTemplate = await TokenTemplate.deploy({
            gasLimit: 3000000,  // Reduced from 5M to 3M
            maxFeePerGas: ethers.parseUnits("50", "gwei"),  // Increased from 35 to 50 gwei
            maxPriorityFeePerGas: ethers.parseUnits("40", "gwei")  // Increased priority fee
        });
        ...
        await initTx.wait();
        console.log("Factory initialized with 0.001 MATIC deployment fee");
