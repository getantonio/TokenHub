...
async function main() {
  ...
  console.log("Deploying V4FactoryWithLiquidityFixedV5 (with token transfer fix) with account:", deployer.address);
  ...
  // Define implementation addresses used in the V4 deployment
  ...
  // Deploy factory with custom distribution support
  console.log("Deploying V4FactoryWithLiquidityFixedV5...");
  const Factory = await ethers.getContractFactory("V4FactoryWithLiquidityFixedV5");
  ...
  console.log("V4FactoryWithLiquidityFixedV5 deployed to:", factoryAddress);
  console.log("");
  ...
  console.log(`NEXT_PUBLIC_${hre.network.name.toUpperCase()}_FACTORY_ADDRESS_V4_WITH_LIQUIDITY_FIXED_V5=${factoryAddress}`);
}
