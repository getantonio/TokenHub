...
async function deployFactory(implementations) {
    ...
    try {
        ...
        console.log("Token Beacon:", addrs[3]);
        console.log("Security Module Beacon:", addrs[4]);
        console.log("Distribution Module Beacon:", addrs[5]);

        console.log("\nVerification Commands:");
        console.log(`npx hardhat verify --network polygonamoy ${factoryAddress} ${deployer.address} ${implementations.tokenImpl} ${implementations.securityModuleImpl} ${implementations.distributionModuleImpl}`);
        ...
        console.log(`NEXT_PUBLIC_POLYGONAMOY_FACTORY_ADDRESS_V4=${factoryAddress}`);

        // Test token creation
        console.log("\nTesting token creation...");
        try {
            const testTx = await factory.createTokenBasic(
                "Test Token",
                "TEST",
