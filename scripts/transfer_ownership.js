...
async function main() {
    // Replace with your new owner address
    const NEW_OWNER = "YOUR_NEW_OWNER_ADDRESS"; // Replace this with your new owner address
    const OLD_OWNER = "0xb6083258E7E7B04Bdc72640E1a75E1F40541e83F";
    ...
    for (const [contractAddress, name] of contracts) {
        try {
            const contract = new ethers.Contract(contractAddress, FACTORY_ABI, signer);
            const owner = await contract.owner();
            
            if (owner.toLowerCase() === OLD_OWNER.toLowerCase()) {
                console.log(`\nContract ${name} (${contractAddress}) is owned by compromised address`);
                console.log("Current owner:", owner);
                ownedContracts.push([contractAddress, name]);
            }
        } catch (error) {
            console.error(`Error checking contract ${name} (${contractAddress}):`, error.message);
        }
    }

    if (ownedContracts.length === 0) {
        console.log("\nNo contracts found owned by the compromised address");
        return;
    }

    console.log("\nFound", ownedContracts.length, "contracts owned by the compromised address");
    
    if (NEW_OWNER === "YOUR_NEW_OWNER_ADDRESS") {
        console.log("\nPlease replace YOUR_NEW_OWNER_ADDRESS with your actual new owner address and run again");
        return;
    }

    console.log("\nTransferring ownership to", NEW_OWNER);
    
    for (const [contractAddress, name] of ownedContracts) {
        try {
            const contract = new ethers.Contract(contractAddress, FACTORY_ABI, signer);
            console.log(`\nTransferring ownership of ${name} (${contractAddress})`);
            const tx = await contract.transferOwnership(NEW_OWNER);
            await tx.wait();
            console.log("Successfully transferred ownership");
        } catch (error) {
            console.error(`Error transferring ownership of ${name} (${contractAddress}):`, error.message);
        }
    }
}
