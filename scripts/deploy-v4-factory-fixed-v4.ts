...
async function main() {
  ...
  console.log("Deploying improved V4FactoryWithLiquidity (v4 with custom distribution) with account:", deployer.address);
  ...
  // Deploy V4FactoryWithLiquidityFixedV4 with the improved implementation
  console.log("Deploying V4FactoryWithLiquidityFixedV4 with custom distribution support...");
  const V4FactoryWithLiquidityFixedV4 = await ethers.getContractFactory("contracts/V4FactoryWithLiquidityFixedV4.sol:V4FactoryWithLiquidityFixedV4");
  const v4Factory = await V4FactoryWithLiquidityFixedV4.deploy(
    deployer.address,
    tokenImpl,
    securityModuleImpl,
    distributionModuleImpl,
  ...
  console.log("V4FactoryWithLiquidityFixedV4 deployed to:", factoryAddress);
  ...
  console.log(`NEXT_PUBLIC_POLYGONAMOY_FACTORY_ADDRESS_V4_WITH_LIQUIDITY_FIXED_V4=${factoryAddress}`);
}
