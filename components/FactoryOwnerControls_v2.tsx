...
export default function FactoryOwnerControlsV2({ isConnected }: FactoryOwnerControlsV2Props) {
  ...
  async function loadFees() {
    ...
    try {
      ...
      setCurrentFee(ethers.formatUnits(fee, 'ether'));

      const balance = await provider.getBalance(factoryAddress);
      setAccumulatedFees(ethers.formatUnits(balance, 'ether'));
    } catch (error) {
  ...
  async function updateDeploymentFee() {
    ...
    try {
      ...
      const tx = await factory.setDeploymentFee(ethers.parseUnits(newFee, 'ether'));
