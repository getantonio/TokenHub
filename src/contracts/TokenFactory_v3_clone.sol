...
import "./TokenTemplate_v3_clone.sol";

contract TokenFactory_v3_clone is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function createToken(
        TokenTemplate_v3_clone.InitParams calldata params
    ) external payable returns (address) {
        require(msg.value >= deploymentFee, "Insufficient deployment fee");

        bytes memory initData = abi.encodeWithSelector(
            TokenTemplate_v3_clone.initialize.selector,
            params
        );
